...
function Tarefas() {
    ...
    const fetchTarefas = async () => {
      try {
        const response = await fetch(
          "https://backand-taskmanager.onrender.com/tasks",
          {
            method: "GET",
            headers: {
  ...
  async function onConfClick(tarefaId) {
    try {
      ...
      const response = await fetch(
        `https://backand-taskmanager.onrender.com/tasks/${tarefaId}`,
        {
          method: "PUT",
  ...
  const handleSaveEdit = async (id, novoTitulo, novaData) => {
    try {
      ...
      const response = await fetch(
        `https://backand-taskmanager.onrender.com/tasks/${id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json", ...authHeaders },
          body: JSON.stringify({
  ...
  const tarefasFiltradas = tarefas
    ...
    .filter((t) => {
      const dataTarefa = t.date?.split("T")[0];
      const isAtrasada = dataTarefa && !t.completed && dataTarefa < hoje;
      if (filtro === "pendentes") return !t.completed && dataTarefa >= hoje;
        ...
        <div className="bg-slate-800/80 backdrop-blur p-6 rounded-2xl space-y-6 md:w-full shadow-lg">
          ...
          <div className="flex flex-col gap-4">
            {tarefasFiltradas.map((tarefa) => {
              ...
              const isAtrasada =
                dataTarefa && !tarefa.completed && dataTarefa < hoje;
